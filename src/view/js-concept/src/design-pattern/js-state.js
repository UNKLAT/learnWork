...
/**
 * 用state模式描述下载，下载行为通常具备以下状态
 * 准备状态（ReadyState）、下载状态（DownloadingState）、暂停状态（DownloadPausedState）、下载完毕状态（DownloadedState）、失败状态（DownloadFailedState），
 * 每个状态都只可以做当前状态才可以做的事情，而不能做其它状态能做的事
 * 
 * state模式的思想就是引入一个State的抽象类来表示表示下载状态，State函数（作为原型）为每个状态的子类（继承函数）声明了一些公共接口。
